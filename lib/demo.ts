...
export function getDemoStats(){
  ...
  const toNotify = subsSoon + certSoon;
  return { subsSoon, certSoon, toNotify };
}

export function listExpirations(){
  const subs = listSubscriptions().map(s=>({nome:s.memberName,tipo:'Abbonamento',scadenza:s.end,stato:'green'}));
  const certs = listCertificates().map(c=>({nome:c.memberName,tipo:'Certificato',scadenza:c.scadenza,stato:'yellow'}));
