...
import { listMembers, addSubscription, listSubscriptions } from '../lib/demo';
    ...
    <>
      ...
      <div className="card">
        <table className="table">
          <thead><tr><th>Atleta</th><th>Piano</th><th>Inizio</th><th>Fine</th></tr></thead>
          <tbody>
            {subs.map((s:any,i:number)=>(
              <tr key={i}><td>{s.memberName}</td><td>{s.piano}</td><td>{s.start}</td><td>{s.end}</td></tr>
            ))}
          </tbody>
        </table>
      </div>
